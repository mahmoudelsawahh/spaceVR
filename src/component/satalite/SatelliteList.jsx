...
import satelliteImage from "../../assets/satalites/satellite-1.svg";
...
const SatelliteList = ({ satList, updateSatelliteList, loading, disabled }) => {
  ...
  return (
    <section>
      <Title level={5} style={{ color: "#eefbfb" }}>
        ...
        renderItem={(item) => (
          ...
          >
            <List.Item.Meta
              ...
              title={
                <>
                  <span>{item.satname}</span>
                  <span className="item-subTitle">{`Launch Date: ${item.launchDate}`}</span>
                </>
              }
            />
          </List.Item>
